...
func TestMissiles(t *testing.T) {
	...
	if shares := q.PerSkill(); len(shares) != 2 || shares[0].Skill != tl.Skill(skillHeat) || shares[0].Missiles.Count() != 3 || shares[1].Missiles.First() != m0 || q.Reverse().Limit(1).First() != m3 || tl.characters[1].Missiles().PerSkill() != nil {
