...
func TestEffects(t *testing.T) {
	...
	if groups := q.GroupBy(func(f *Effect) bool { return f.Ground }); groups[true].Count() != 2 || groups[false].First() != f2 || p2.Effects().PerID() == nil || tl.characters[1].Effects().Reverse().PerID()[0].Effects.First() != f2 {
...
func TestEffectWindows(t *testing.T) {
	...
	if tl.Effects().Skip(1).First().EffectID != 5001 || tl.Effects().Skip(2).PerID() != nil || tl.characters[1].Effects().PerID() != nil {
...
func TestUntrackedEffects(t *testing.T) {
	...
	tl := mustBuild(t, b.build(10000))
	all := tl.characters[0].Effects().All()
	if len(all) != 5 {
		t.Fatalf("effects = %d", len(all))
	}
