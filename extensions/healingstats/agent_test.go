...
func TestAgentNode(t *testing.T) {
	...
	if s.Agent(nil) != nil || s.Agent((*timeline.Player)(nil)) != nil || s.Agent((*Agent)(nil)) != nil || s.Agent(alpha) != alpha || s.Agent(alpha.Agent) != alpha || len(s.Agents) != len(tl.Agents) || s.Players[0] != alpha || s.Unknown.Agent != tl.Unknown || s.Agent(tl.Unknown) != s.Unknown {
		t.Error("lookups are inconsistent")
	}
	other := mustBuild(t, filterLog())
	if s.Agent(other.Players[0]) != nil || other.Agent(alpha) != nil {
